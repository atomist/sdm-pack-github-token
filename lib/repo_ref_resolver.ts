...
import { NewTokenGitHubRepoRef } from "./repo_ref";

export class NewTokenGitHubRepoRefResolver implements RepoRefResolver {
	/**
	...
	public repoRefFromPush(push: OnPushToAnyBranch.Push): RemoteRepoRef {
		...
		switch (providerType) {
			...
			case ProviderType.ghe:
				return NewTokenGitHubRepoRef.from({
			...
			default:
				throw new Error(
					`Provider ${providerType} not currently supported in SDM`,
				);
		}
	}
	...
	public repoRefFromSdmGoal(sdmGoal: SdmGoalEvent): RemoteRepoRef {
		...
		switch (provider.providerType) {
			...
			default:
				throw new Error(
					`Provider ${provider.providerType} not currently supported in SDM`,
				);
		}
	}
	 ...
	 */
	public toRemoteRepoRef(
		repo: CoreRepoFieldsAndChannels.Fragment,
		...
		switch (providerType) {
			...
			default:
				throw new Error(
					`Provider ${repo.org.provider.providerType} not currently supported in SDM`,
				);
		}
	}
}
